...
当前 Bright 余额只读查询为 `$8.96`，这会直接改变首轮验证策略：Bright 不能被设计成 benchmark 的主要消耗池，只能作为极小样本对照组、结构化 profile probe 和高潜 URL 补全验证。首轮不默认充值，不把 Bright 扩量当成解决召回质量的办法。
...
0. 首轮 benchmark 在当前 `$8.96` 余额下，Bright 子预算硬上限建议为 `$1`，只做极小 profile completion / Dataset Filter 对照；超过这个上限必须单独确认。
...
| 首轮内部 benchmark | 总预算 `$50` 硬上限；Bright 子预算建议 `$1` 硬上限 | 优先免费额度、小额 SERP/Exa/Firecrawl，Bright 只做极小 probe 和对照组 |
...
首轮内部验证不是正式产品成本模型，目标是用 `$50` 以内判断路线是否值得继续。由于当前 Bright 余额只读查询为 `$8.96`，首轮不能把 `$50` 理解成 Bright 可用预算；Bright 消耗要单独设为约 `$1` 上限，并在每次真实调用前先做只读余额检查。这个预算下如果完全无法产出 reviewable profiles，应该优先判定数据源/召回路线有问题，而不是直接加预算掩盖问题。
...
| Bright Dataset Filter | LinkedIn structured profile 小额 probe | `BRIGHTDATA_API_TOKEN`、`BRIGHTDATA_DATASET_ID` | `.env` 已配置；`.env.local` 有 token；余额只读查询为 `$8.96` | 可选但建议验证 | 已有；使用前先查余额和 dataset 权限；首轮 Bright 子预算建议 `$1` 硬上限，避免误触发大额任务 |
...
如果预算严格卡在 `$50`，优先不要新开有最低充值的平台。DataForSEO 这类低单价但有 minimum payment 的服务，应等 Serper 免费/低成本额度不足时再启用。Bright 当前余额只有 `$8.96`，所以首轮不能靠 Bright 扩量补召回，只能用极小 probe 判断它是否值得后续充值或替换。
...
| Bright 消耗 | 当前余额 `$8.96`；首轮 Bright 子预算建议不超过 `$1`，超过必须单独确认 |
