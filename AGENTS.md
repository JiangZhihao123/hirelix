...
- 核心链路验证应优先使用真实登录/session、真实数据库、真实 API 路由、真实调度器、真实 LLM/Bright Data/GitHub 等外部依赖
- 生产/准生产链路测试优先在 `us-2` 上执行，尤其是 Bright Data、GitHub、LLM、Postgres 等外部依赖链路；本地中国大陆网络容易放大跨境下载/连接耗时，不能单独作为生产性能结论
