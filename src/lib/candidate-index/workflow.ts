...
export async function runCandidateIndexWorkflow(params: {
  ...
  const qualificationConcurrency = Math.max(1, Math.min(8, Number(process.env.SEARCH_QUALIFICATION_CONCURRENCY || 6)));
  ...
  const finalJudgments = await runWithConcurrency(topBundles, 4, (bundle) =>
